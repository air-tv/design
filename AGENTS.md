...
- Mock records must instantiate the public models from the sibling `stremio-addon-client` and `iptv` KMP builds. Do not recreate protocol/domain models inside this repository.
...
- The sibling builds are included through Gradle composite substitution; Maven coordinates remain `com.getair:stremio-addon-client` and `com.getair:iptv` so the app can switch to published artifacts without source changes.
